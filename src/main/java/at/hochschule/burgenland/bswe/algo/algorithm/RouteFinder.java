...
import at.hochschule.burgenland.bswe.algo.algorithm.finderutil.DijkstraNode;
import at.hochschule.burgenland.bswe.algo.algorithm.finderutil.WeightFunction;
...
public class RouteFinder {
  ...
  private Route dijkstra(Airport origin, Airport destination, WeightFunction weightFunc) {
    ...
    PriorityQueue<DijkstraNode> priorityQueue = new PriorityQueue<>(Comparator.comparingDouble(n -> n.weight));
    Map<String, Double> distances = new HashMap<>();

    priorityQueue.offer(new DijkstraNode(0.0, origin.getIata(), new ArrayList<>()));
    ...
    while (!priorityQueue.isEmpty()) {
      ...
      for (Flight flight : outgoingFlights) {
        ...
        if (newWeight < distances.getOrDefault(nextIata, Double.MAX_VALUE)) {
          ...
          priorityQueue.offer(new DijkstraNode(newWeight, nextIata, newPath));
        }
      }
    }
